...
class TestRepository extends AbstractRepository implements TestRepositoryInterface
{
    ...
    public function getList(?int $company_id): array
    {
        ...
        $result = Test::query()
            ->select([
                'tests.*',
                DB::raw(' (select count(id) from questions where
                        test_id = tests.id)as question_count'),
                DB::raw(' (select round(avg(score)) from scores where
                        test_id = tests.id group by test_id)as avg_score'),
            ]);
